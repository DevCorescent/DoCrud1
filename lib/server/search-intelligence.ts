/**
 * Query understanding for Docrud global search.
 *
 * Turns a natural-language sentence ("I need a React developer in Delhi who can
 * build a SaaS platform") into structured signals the ranker can use.
 *
 * Two layers, in this order:
 *   1. Deterministic rules — stopword stripping, intent detection, a curated
 *      concept taxonomy for synonym/related-term expansion, and location
 *      extraction validated against locations that actually exist in the data.
 *      This layer always runs and never fails.
 *   2. Optional LLM enrichment through the project's existing Groq client
 *      (lib/server/ai.ts). It only ever *adds* terms. If GROQ_API_KEY is absent,
 *      the call times out, quota is exhausted, or the JSON is malformed, the
 *      rules result is returned unchanged.
 *
 * There are no embeddings and no vector database in this project, so "semantic"
 * here means concept expansion over a curated taxonomy, not vector similarity.
 * That is a deliberate choice — see the report — not a stand-in that pretends
 * to be embeddings.
 */
